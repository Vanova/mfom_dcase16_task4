...
class UvZMisclassification(_Merge):
    ...
    def _merge_function(self, inputs):  # TODO note NO inference stage, only training
        y_true = inputs[0]
        y_pred = inputs[1]
    ...
    def _uvz_misclass(self, y_true, y_pred):
        y_pred = K.clip(y_pred, self._EPSILON, 1.0 - self._EPSILON)
        y_neg = 1 - y_true
...
class SmoothErrorCounter(Layer):
    ...
    def build(self, input_shape):
        self.axis = 1
        dim = input_shape[self.axis]  # [batch_sz, nclasses]
        if dim is None:
